...
def parse_to_braille_cells(text):
    ...
    while i < L:
        ...
        if ch in SPECIAL_TO_BRAILLE:
            braille_cells.append(SPECIAL_TO_BRAILLE[ch])
            i += 1
            continue
        if ch.isalpha():
            if ch.isupper():
        ...
        if ch.isdigit():
            ...
            continue
        if ch.isspace():
            braille_cells.append([0,0,0,0,0,0])  # 빈 셀
        else:
            braille_cells.append([0,0,0,0,0,0])
        i += 1
...
if __name__ == "__main__":
    ...
    text = input('한글/영문/숫자/특수문자/축약어 입력: ')
